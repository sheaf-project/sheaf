...
- **API keys can no longer set the "I am 18 or older" attestation.** The attestation is one-way, cannot be undone, and is what unlocks creating share grants, so a key of any scope being able to set it meant a leaked key could clear the gate in front of publishing. It is now session/JWT only, and setting it is recorded in the account's security-event trail with the time and originating address. This adds a new security-event type and a database migration.
