...
### Finding the image digest for Rekor lookups

If you want to cross-reference a build against the public Rekor transparency log directly (independent of cosign / GitHub):

1. Get the running commit from `/v1/version` or from the instance's `/about` page.
2. Run `cosign verify ghcr.io/sheaf-project/sheaf:sha-<commit> --certificate-identity-regexp "https://github.com/sheaf-project/sheaf/.github/workflows/ci.yml@.*" --certificate-oidc-issuer https://token.actions.githubusercontent.com`. The output includes a JSON blob; the `critical.image.docker-manifest-digest` field is the image's `sha256` digest.
3. Visit [search.sigstore.dev](https://search.sigstore.dev/), choose the **Hash** tab, paste the digest (with or without the `sha256:` prefix). You'll get the Rekor log entry that records this image's signing.

A note on the **Commit SHA** tab on that page: it only finds entries whose attestation predicate has a structured `commit_sha` field — typically SLSA provenance generated by `slsa-github-generator`. Sheaf currently signs with plain `cosign sign`, where the commit lives on the signature certificate (X.509 extension `1.3.6.1.4.1.57264.1.13`, source-repository-digest) but isn't in Rekor's primary index. Pasting a git commit into the Commit SHA tab won't find Sheaf entries today; switching to SLSA provenance to fix that is queued as future work.
...
### How to verify (in-browser, easiest)

Open `/about` on your Sheaf instance and click **Verify this page** in the Bundle integrity card. The browser will re-fetch every file in the manifest, compute its SHA-384 in your browser via the Web Crypto API, and compare to the manifest's recorded integrity. Results render per-file with a pass/fail icon. The hash function and comparison both live in your browser, not on the server, so this is a real verification of "what bytes did this server actually send me" — the server can only influence the bytes it serves, which is the thing being checked.

This covers `index.html` too — closing the "SRI doesn't protect the document that carries the SRI attributes" bootstrap gap, at least for verification. (Browser-enforced *loading* of `index.html` against a hash still needs the cosign-attested manifest path; see below.)

### How to verify (manual, browser-side)

If you want to do it without trusting any in-page JavaScript:
