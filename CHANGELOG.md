...
- **OpenPlural files now bring relationships and share views back in with them.** The OpenPlural exporter has always written your relationship types, member relationships, group relationships and share views into the file, but the importer dropped all four sections on the way back in - so a round-trip through the interchange format lost them, silently. They are restored now, and the deselect-a-section toggle for relationships is honoured on that path too. (Native Sheaf backups were never affected.) Share views come back exactly as they do from a native backup: curated, but published to nobody, because a share link or public profile is never written to a portable file in the first place.
- **Restoring a backup no longer silently drops a share view whose name is already taken.** A view whose name collides with one you already have is skipped whole rather than merged - deliberately, since merging could add members to a view that is already shared with someone. That skip left no trace, so the view simply wasn't there afterwards. The import report now names the view it skipped and says why, and counts imported and skipped share views alongside every other section. A hand-edited or corrupted file whose share-view entries are malformed, or whose view name is over the 100-character limit, is now reported the same way instead of being dropped or trimmed in silence.
