...
scope: account ids and system ids are PFADDed in at the auth choke point, and
only the estimated COUNT is ever published, as an id-free gauge. HLL is one-way
- you cannot enumerate members or answer "was account X active on day Y" from a
sketch - so raw ids are NEVER stored anywhere, only folded into the registers.
...
cardinality of the UNION of 30 daily sketches; it cannot be reconstructed by
...
missing day-key is RESTOREd from Postgres before the monthly union is computed.
...
def day_key(scope: str, day: date) -> str:
    """Redis key for a scope's day-sketch, e.g. sheaf:hll:acct:2026-09-01."""
    return f"sheaf:hll:{scope}:{_day_str(day)}"
...
def record_active_account(user_id: uuid.UUID) -> None:
    """Record an authenticated account (and its system) as active today.

    Called from the auth dependency once a request has authenticated, for every
    auth method (API key, JWT, session cookie). Synchronous and non-blocking: it
    only schedules a background task and returns immediately, so neither Redis
    latency nor a Redis outage can delay or fail the request. Any error inside
    the task is swallowed - usage metrics are strictly best-effort.
    """
    try:
        ...
        task = loop.create_task(_record_active(user_id))
        _bg_tasks.add(task)
...
async def _record_active(user_id: uuid.UUID) -> None:
    """Fire-and-forget body: PFADD the account id into today's acct sketch and
    the system id into today's sys sketch, refreshing the ~31-day TTL. Uses the
    existing shared Redis client. Never raises."""
    try:
...
async def flush_day_sketches(db: AsyncSession) -> dict:
    ...
    Reads the raw sketch bytes off each recent day-key in Redis (bytes-mode
    client - the registers are binary, not UTF-8) and UPSERTs them keyed by
    (day, scope). This is the durability backstop: after a Redis instance
    replace wipes the day-keys, the monthly union restores them from here. Only
    the aggregate registers are stored, nothing per-account, so this table is
    ops data and stays out of the user data export.
    ...
    if rb is not None:
        ...
        # candidate day/scope pairs and flush any that currently exist.
        for day in _recent_days(SKETCH_RETENTION_DAYS):
            for scope in SCOPES:
                try:
                    raw = await rb.get(day_key(scope, day))
                except Exception:
                    logger.debug(
                        "usage: reading sketch bytes failed for %s %s",
                        scope, day, exc_info=True,
                    )
                    continue
                if raw is None:
                    continue
...
async def daily_count(scope: str) -> int | None:
    """Estimated distinct-id cardinality for today's sketch (PFCOUNT).

    Returns None if Redis is unreachable, so the caller can leave the gauge at
    its last value rather than zeroing it.
    """
    try:
        from sheaf.auth.sessions import get_redis_bytes

        rb = await get_redis_bytes()
        return int(await rb.pfcount(day_key(scope, _today())))
    except Exception:
        logger.debug("usage: daily PFCOUNT failed for %s", scope, exc_info=True)
        return None


async def monthly_count(db: AsyncSession, scope: str) -> int | None:
    """Estimated distinct-id cardinality over the trailing 30 days.

    MAU is the cardinality of the UNION of the 30 daily sketches (PFMERGE then
    PFCOUNT), NOT the sum of daily counts - summing double-counts anyone active
    on more than one day. Where a day-key is MISSING from Redis (the post-replace
    case), its persisted sketch is RESTOREd from Postgres into a scratch key so
    it still participates in the union. This restore path is the entire reason
    the sketch bytes are persisted: without it a Redis replace would silently
    drop up to 30 days of history from MAU.
    ...
    scratch = f"sheaf:hll:scratch:{scope}:{uuid.uuid4().hex}"
    ...
    try:
        ...
        # Which day-keys are actually present in Redis right now.
        present: dict[date, bool] = {}
        for day in days:
            try:
                present[day] = bool(await rb.exists(day_key(scope, day)))
            except Exception:
                present[day] = False

        missing_days = [d for d in days if not present[d]]

        # Restore missing days from the persisted Postgres sketches. This is the
        # post-Redis-replace recovery: without it the union would silently omit
        # every day whose key Redis lost.
        if missing_days:
            restored_bytes = await _load_persisted_sketches(db, scope, missing_days)
            for day, raw in restored_bytes.items():
                tmp = f"sheaf:hll:restore:{scope}:{_day_str(day)}:{uuid.uuid4().hex}"
                try:
                    ...
                    logger.debug(
                        "usage: restoring persisted sketch failed for %s %s",
                        scope, day, exc_info=True,
                    )
        ...
        for day in days:
            if present[day]:
                source_keys.append(day_key(scope, day))
        ...
        logger.debug("usage: monthly union failed for %s", scope, exc_info=True)
        return None
    finally:
...
async def _load_persisted_sketches(
    db: AsyncSession, scope: str, days: list[date]
) -> dict[date, bytes]:
    """Fetch persisted sketch bytes for the given (scope, days) from Postgres."""
    from sheaf.models.usage_sketch import UsageDailySketch
...
async def refresh_usage_gauges(db: AsyncSession) -> None:
    """Set the id-free DAU/MAU cardinality gauges.
    ...
    acct_daily = await daily_count(SCOPE_ACCOUNT)
    if acct_daily is not None:
        active_accounts_daily.set(acct_daily)

    sys_daily = await daily_count(SCOPE_SYSTEM)
    if sys_daily is not None:
        active_systems_daily.set(sys_daily)

    acct_monthly = await monthly_count(db, SCOPE_ACCOUNT)
    if acct_monthly is not None:
        active_accounts_monthly.set(acct_monthly)

    sys_monthly = await monthly_count(db, SCOPE_SYSTEM)
    if sys_monthly is not None:
        active_systems_monthly.set(sys_monthly)
